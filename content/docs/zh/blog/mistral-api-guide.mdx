  ...
  json='{
    ...
    "mainEntity": [
      ...
      },
      {
        ...
        "acceptedAnswer": {
          "@type": "Answer",
          "text": "您可以通过 TokenPAPA（https://tokenpapa.ai）从海外访问 Mistral AI API。TokenPAPA 提供 OpenAI 兼容的端点 https://api.tokenpapa.ai/v1 — 只需用邮箱注册、充值、生成 API 密钥即可开始调用 Mistral 模型，无地域限制、无需电话验证、支持国际支付。"
        }
      },
      {
        "@type": "Question",
...
使用 API 中继平台。[TokenPAPA](https://tokenpapa.ai) 为全球用户提供 Mistral API 访问，无地域限制。使用邮箱注册（无需电话验证），通过信用卡/PayPal/加密货币充值，生成 API 密钥，然后使用 `https://api.tokenpapa.ai/v1` — 设置过程不到 3 分钟。同一个密钥还可以访问 200+ 其他模型。
