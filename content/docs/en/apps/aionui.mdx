...
description: AionUi Tutorial — A free open-source desktop office agent supporting various AI agents like Gemini CLI, Claude Code, Codex, and more. Integrates with TokenPAPA to enable multi-model workflows.
---
...
Provider Type: Types supported by TokenPAPA
API Key: Obtained from TokenPAPA
API Address: TokenPAPA site address (e.g., `https://<your-newapi-domain>/v1`)

Please use your self-deployed TokenPAPA, or ensure that the service provider offers TokenPAPA services with legitimate upstream authorization and compliance obligations. Do not connect API addresses or keys from unknown sources to a production environment.

### Configuration Steps

1.  **Copy API key in TokenPAPA**
...
3.  **Add New Provider**
    - Click "Add Model"
    - Select TokenPAPA
    ![Add TokenPAPA Provider](/assets/aionui/newapi_provider.png)

4.  **Configure API Information**
    - API Address: Enter your TokenPAPA site address (format: `https://<your-newapi-domain>/v1`)
    - API Key: Paste the API Key copied from the TokenPAPA console

5.  **Add Model**
    - Select the model to add from the dropdown
    - The model name should match the model name configured in TokenPAPA
    - Select the appropriate request protocol

6.  **Start Using**
    - Return to the chat page
    - Select the configured TokenPAPA model to start a conversation
