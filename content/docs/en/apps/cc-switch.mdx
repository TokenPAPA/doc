...
description: CC Switch tutorial — a cross-platform tool for unified management of Claude Code, Codex & Gemini CLI provider configs, MCP servers, and system prompts. One-click import from TokenPAPA.
---

<Callout type="info" title="Chat Settings Option">
  In the TokenPAPA console under System Settings -> Chat Settings, you can add the following shortcut to enable one-click fill for CC Switch from the token management page:
</Callout>
...
CC Switch supports the `ccswitch://` Deep Link protocol, allowing one-click provider config import from the TokenPAPA token management page.

### Configuration Steps

1. **On the TokenPAPA token management page, click the dropdown menu for the corresponding token**
