...
description: "Access Moonshot Kimi API from overseas in 2026. Complete guide to Kimi k1.5 model capabilities, long context window features, pricing, and integration."
...
4. **Use the endpoint** `https://tokenpapa.ai/v1` with any OpenAI-compatible client
...
|--------------|:-----:|-------|
| **LangChain** | Set `base_url` to `https://tokenpapa.ai/v1` | Full support for chains, agents, tools |
...
**Yes — fully compatible.** Any existing OpenAI client code works by changing the `base_url` to `https://tokenpapa.ai/v1` and swapping the API key. The same applies to LangChain, LlamaIndex, Vercel AI SDK, and any other tool that supports OpenAI-compatible APIs. Switching from Moonshot K2 to DeepSeek V3 or any other Chinese LLM requires changing only the `model` parameter.

### 7. What Chinese LLM models are available besides Moonshot AI via TokenPAPA?

[TokenPAPA](https://tokenpapa.ai) provides access to **DeepSeek V3**, **DeepSeek R1**, **Qwen 2.5** (72B, Coder 32B, Math 72B), **GLM-4** (including GLM-4V and GLM-4 32K), **MiniMax Text-01**, and **Moonshot AI** — all through a single API key and the same OpenAI-compatible endpoint at `https://tokenpapa.ai/v1`.
